  ...
  <div className="w-full h-screen flex items-center justify-center">
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="flex gap-2">
        {[0, 1, 2].map((i) => (
          <div key={i} className="w-4 h-4 bg-green-600 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.1}s` }}></div>
        ))}
      </div>
...
export default function Products({ userData }) {
  ...
  if (catLoading || isLoading) return <LoadingAnimation />;
            ...
            <>
              ...
              <div className="flex text-[20px] items-center py-3 my-3 font-[500] relative">
                {catData?.data?.map((data, index) => (
                  data.isListed && (
                    <p key={index} 
                      style={{ opacity: cPosition === index ? "100%" : "40%", cursor: isLoading ? "wait" : "pointer" }}
                      onClick={() => handleCategoryChange(index)}
                      className={`w-40 transition-opacity capitalize duration-300 m-0 leading-none ${isLoading ? "pointer-events-none" : ""}`}
                    >
                      {data.name}
                    </p>
                  )
                ))}
                <div style={{ left: `${160 * cPosition}px` }} className="w-16 h-1 duration-500 bg-[#44764850] absolute bottom-0" />
              </div>
                  ...
                  <>
                    ...
                    <div className="w-full h-auto flex my-5 mt-12 gap-5 relative">
                      {productsData?.data?.length > 14 && (
                        <ViewAllButton 
                          onClick={() => navigator(`/user/collection/${catData?.data[cPosition].name}/products`, {
                            state: {
                ...
                {productsData?.data?.length > 0 && (
                  <>
                    ...
                    <div className="w-full h-auto flex my-5 mt-8 gap-5 mb-80 relative flex-wrap">
                      {productsData?.data?.length > 14 && <ViewAllButton onClick={() => navigator(`/user/collection/${catData?.data[cPosition].name}/products`, {
                        state: {
                          products: CollData?.data,
                          action: "collections",
                          title: `Collections of ${catData?.data[cPosition].name}`
...
const ViewAllButton = ({ onClick }) => (
  <div onClick={onClick} className="px-8 items-center justify-center group flex duration-500 absolute font-medium right-0 top-[-65px] py-2 bg-[linear-gradient(to_left,#52aa5799,#14532d)] hover:scale-125 text-white tex-[20px] gap-2 rounded-l-[10px] rounded-bl-[20px]">
