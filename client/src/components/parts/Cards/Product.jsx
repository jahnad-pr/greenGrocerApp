...
const Product = ({ pos, data, type, userData }) => {
      ...
      <span className="w-full h-auto bg-[linear-gradient(#ffffff40,#ffffff70)] flex flex-col px-10 rounded-[30px] md:rounded-br-[120px] rounded-br-[100px] pt-10 flex-1 justify- gap-2 pb-0">
        <span className="mt-2">
          ...
          <span className="flex flex-col">
            <s>
              <p className="opacity-30">₹ {data?.regularPrice}</p>
            </s>
            <p className="opacity-60 md:text-[25px] text-[20px] font-bold text-[#14532d]">
              ₹{' '}
              {(
                data?.regularPrice -
                (data?.discount?.isPercentage
                  ? (data?.regularPrice * data?.discount?.value) / 100
            ...
            {userData?._id && (
              <div className="relative">
                <img
                  src={isMarked ? '/hearted.svg' : '/heart.svg'}
                  onClick={() =>
                    isMarked
                      ? bookmarkHandler(data._id, 'remove')
                      : bookmarkHandler(data._id, 'add')
                  }
                  className={`w-20 h-20 opacity-45 absolute top-28 right-0 rounded-full p-5 hover:scale-125 duration-500 ${
        ...
        <button
          onClick={() =>
            type === 'product'
              ? navigate('/user/productPage', { state: { id: data._id } })
              : navigate(`/user/collection/${data.name}/products`, {
                  state: { products: data?.products, action: 'collection' },
                })
          }
          className="flex justify-start items-center font-bold rounded-full text-white absolute bottom-0 md:right-3 right-3 bg-[linear-gradient(#b4c2ba,#789985)] overflow-hidden w-[40px] h-[40px] md:w-[70px] md:h-[70px] hover:scale-125 duration-500 group"
        >
          <img
            className="group-hover:-translate-x-full w-[40px] h-[40px] md:w-[70px] md:h-[70px] md:p-4 p-2 brightness-[100] duration-500"
            src="/bag-2-1.svg"
            alt=""
          />
          <img
            className="group-hover:-translate-x-full w-[40px] h-[40px] md:w-[70px] md:h-[70px] p-2 md:p-5 brightness-[100] duration-500"
            src="/arrow-right.svg"
            alt=""
          />
        </button>
