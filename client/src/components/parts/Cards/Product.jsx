...
export default function Product({pos,data,type,userData}) {
  ...
  return (
    <div className={`h-80 md:min-w-56 min-w-44 max-w-44 md:max-w-56 flex flex-col justify-center items-center rounded-[40px] relative`}>
        ...
        {userData?._id &&isLoading? <img src={isMared?'/hearted.svg':'/heart.svg'} onClick={()=>isMared?bookmarkHandler(data._id,'remove'):bookmarkHandler(data._id,'add')} className={`w-20 h-20 opacity-45 absolute top-28 right-0 rounded-full p-5 hover:scale-125 duration-500 `}></img>:
